...
void ast_walker_walk_block(AstWalker *aw, Node *node) {
    ...
    BlockData *bd     = &aw->data.m.extra_data.items[ed_idx].data.block;
    Block      block  = {.main_token = node->main_token, .bd = bd};
    bool continue_ = SAFE_CALLBACK_CALL(aw->block, aw->user_data, &aw->data, block);
    ...
    for (usz i = 0; i < bd->count; i++) {
        Node *block_node = &aw->data.m.nodes.items[bd->items[i]];
...
void ast_walker_walk_function_definition(AstWalker *aw, Node *node) {
    ...
    FunctionPrototypeData *fpd =
        &aw->data.m.extra_data.items[ed_idx].data.function_prototype;
    FunctionDefinition fd = {
        .block = node->data.rhs, .main_token = node->main_token, .fpd = fpd};
    bool continue_ = SAFE_CALLBACK_CALL(aw->function_definition, aw->user_data, &aw->data, fd);

    if (!continue_) {
        return;
    }

    Node *block_node = &aw->data.m.nodes.items[node->data.rhs];
    ast_walker_walk_block(aw, block_node);
}
void ast_walker_walk_integer(AstWalker *aw, Node *node) {
    TokenExtraData *ted     = &aw->data.t.extra_data.data[node->main_token];
    int             literal = ted->data.integer;
    IntegerLiteral  il = {.integer = literal, .main_token = node->main_token};

    SAFE_CALLBACK_CALL(aw->integer_literal, aw->user_data, &aw->data, il);
}
void ast_walker_walk_variable_declaration(AstWalker *aw, Node *node) {
    VariableDeclaration vd = {.main_token = node->main_token,
                              .type       = node->data.lhs,
                              .expr       = node->data.rhs};
    bool continue_ =  SAFE_CALLBACK_CALL(aw->variable_declaration, aw->user_data, &aw->data, vd);

    if (!continue_) {
        return;
    }

    Node *expr_node = &aw->data.m.nodes.items[node->data.rhs];
    ast_walker_walk_node(aw, expr_node);
}
void ast_walker_walk_eof(AstWalker *aw, Node *node) {
    Eof eof = {.main_token = node->main_token};

    SAFE_CALLBACK_CALL(aw->eof, aw->user_data, &aw->data, eof);
}
...
void ast_walker_walk_top_level_node(AstWalker *aw, usz top_level_node) {
    Index node_idx = aw->data.m.top_level_nodes.items[top_level_node];
    Node *node     = &aw->data.m.nodes.items[node_idx];

    ast_walker_walk_node(aw, node);
}

void ast_walker_walk(AstWalker *aw) {
    for (usz i = 0; i < aw->data.m.top_level_nodes.count; i++) {
